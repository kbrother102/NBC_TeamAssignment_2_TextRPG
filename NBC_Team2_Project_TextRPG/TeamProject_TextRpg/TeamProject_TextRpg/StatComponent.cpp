...
void StatInfo::AddExp(int amount)
{
    exp += amount;
    // 로그에 기록 (디버깅용)
    Logger::Add(INFO, name + "(이)가 경험치를 " + std::to_string(amount) + " 얻었습니다.");
    ...
    while (exp >= maxExp)
    {
        ...
        Logger::Add(COMBAT, "====== [ 레벨 업! (" + std::to_string(level) + ") ] ======");
    }
}
...
{
    gold += amount;
    Logger::Add(INFO, std::to_string(amount) + " G를 획득했습니다. (현재: " + std::to_string(gold) + " G)");
}